Public Class ctrlSuaBangBaoGia
    ...
    Public Sub SuaPhieuBaoGia(BaoGia As tbBaoGia)
        ...
        If BaoGia.idNguoiLienHe IsNot Nothing Then
...
#Region "CẬP NHẬT SỐ PHIẾU"
    ...
    Public Sub CtrlTimKiemSanPham1_ChonSanPhamThanhCong(SanPham As tbSanPham, SoLuong As Double, TheoDonVi As Boolean) Handles CtrlTimKiemSanPham1.ChonSanPhamThanhCong
        If txtKhachHang.Tag Is Nothing Then
            ThongBao("Nhập khách hàng trước khi nhập danh sách sản phẩm")
            Exit Sub
        End If
    ...
    Public Sub btnDongY_Click(sender As Object, e As EventArgs) Handles btnDongY.Click
        ...
        If txtKhachHang.Tag Is Nothing Then
            ...
            Exit Sub
        End If
        If ComponentFactory.Krypton.Toolkit.KryptonMessageBox.Show("Xác nhận sửa bảng báo giá " + txtSoPhieuNhap.Text, "Xác nhận", MessageBoxButtons.YesNo, MessageBoxIcon.Question) <> DialogResult.Yes Then
            Exit Sub
        End If
        With BaoGia
            ...
            .VAT = nudVAT.Value
            .tbLoaiBaoGia = cmbLoaiBaoGia.SelectedItem
    ...
    Private Sub btnChuyenDonDatHang_Click(sender As Object, e As EventArgs) Handles btnChuyenDonDatHang.Click
        If dt.tbBaoGias.First(Function(s) s.id = BaoGia.id).isHuy = True Then
            ThongBao("Phiếu này đã được hủy")
            Exit Sub
        End If
        Dim frm = New frmKhachHangDatHang
        frm.BaoGia = dt.tbBaoGias.First(Function(s) s.id = BaoGia.id)
        Dim vBaoGia As vwBaoGia = dt.vwBaoGias.First(Function(s) s.id = BaoGia.id)
        Dim KhachHangDatHang = dt.vwKhachHangDatHangs.FirstOrDefault(Function(s) s.idBaoGia = vBaoGia.id)
        If KhachHangDatHang IsNot Nothing Then
            ...
            End If
        End If
        frm.TenKhachHangDatHang = If(BaoGia.isNhapHang, "PHIẾU ĐẶT HÀNG", "ĐƠN ĐẶT HÀNG")
        frm.Show()
    End Sub
...
#End Region
    ...
    Public Sub chkAn_Click(sender As Object, e As EventArgs) Handles chkAn.Click
    ...
    Private Sub btnInPhieuBaoGia_Click(sender As Object, e As EventArgs) Handles btnInPhieuBaoGiaNCC.Click, btnInPhieuNoiBo.Click
        If lstChiTietBaoGia.Count = 0 Then
            ComponentFactory.Krypton.Toolkit.KryptonMessageBox.Show("Danh sách sản phẩm báo giá rỗng.", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Exit Sub
        End If
        ...
        Try
            ...
            If sender.Tag IsNot Nothing Then
                TaoBaoGiaXuatExcel1(Application.StartupPath + "\UyNhiemChi\BaoGia1.xlsx", dt.vwBaoGias.Single(Function(s) s.id = BaoGia.id), False)
            Else
                TaoBaoGiaXuatExcel1(Application.StartupPath + "\UyNhiemChi\BaoGia1.xlsx", dt.vwBaoGias.Single(Function(s) s.id = BaoGia.id), True)
            End If
        Catch
            ComponentFactory.Krypton.Toolkit.KryptonMessageBox.Show("Sửa phiếu thất bại. Vui lòng thử lại", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information)
            undoChange()
