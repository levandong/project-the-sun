Public Class ctrlSuaKhachHangDatHang
    ...
    Private Sub cmbNguoiLienHe_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbNguoiLienHe.SelectedIndexChanged
        ...
        End If
    End Sub
    ''Cập nhật danh sách sản phẩm khách hàng
    Public Sub XemPhieu(KhachHangDatHang As tbKhachHangDatHang)
        'ChonKhachHang(dt.tbKhachHangs.First(Function(s) s.id = KhachHangDatHang.idKhachHang))
        txtKhachHang.Tag = KhachHangDatHang.tbKhachHang
        txtKhachHang.Text = KhachHangDatHang.tbKhachHang.TenKhachHang
        cmbNhanVien.DataSource = dt.tbNhanViens.Where(Function(s) s.id = KhachHangDatHang.idNhanVien)
        Chon.Visible = False
        cmbNguoiLienHe.DataSource = dt.tbChiTietLienHes.Where(Function(s) s.id = KhachHangDatHang.idNguoiLienHe)
        If cmbNguoiLienHe.SelectedItem IsNot Nothing Then
            cmbNguoiLienHe_SelectedIndexChanged(Nothing, Nothing)
        End If
        txtSoPhieuNhap.Tag = KhachHangDatHang.SoPhieu
        dtpNgayDuKienGiaoHang.Value = KhachHangDatHang.NgayDuKienGiaoHang
        chkXuatHoaDon.Checked = KhachHangDatHang.isHoaDon
        txtDiaChi.Text = If(KhachHangDatHang.tbKhachHang IsNot Nothing, KhachHangDatHang.tbKhachHang.DiaChi, "")
        txtDiaChiGiaoHang.Text = KhachHangDatHang.DiaChiGiaoHang
        If KhachHangDatHang.tbKhachHang IsNot Nothing Then
            txtDiaChi.Text = KhachHangDatHang.tbKhachHang.DiaChi
        End If
        nudVAT.Value = KhachHangDatHang.VAT
        isNhapHang = KhachHangDatHang.isNhapHang
        dtpNgayLap.Value = KhachHangDatHang.NgayLap
        txtCongTrinh.Text = KhachHangDatHang.CongTrinh
        txtGhiChu.Text = KhachHangDatHang.GhiChu
        lstChiTietDatHang = (From itm In dt.tbChiTietKhachHangDatHangs
                             Where itm.idKhachHangDatHang = KhachHangDatHang.id).ToList
        If dt.tbPhieuNhaps.Where(Function(s) s.idKhachHangDatHang = KhachHangDatHang.id).Count > 0 Then
            For Each itm In dt.tbPhieuNhaps.Where(Function(s) s.idKhachHangDatHang = KhachHangDatHang.id)
                For Each im In dt.tbChiTietPhieuNhaps.Where(Function(s) s.idPhieuNhap = itm.id)
                    lstChiTietPhieuNhap.Add(im)
                Next
            Next
            Dim rls = From itm In lstChiTietPhieuNhap
                      Group By itm.idSanPham Into SoLuong = Sum(itm.SoLuong)
            For Each itm In rls
                Dim Chitiet = lstChiTietDatHang.Single(Function(s) s.idSanPham = itm.idSanPham)
                Chitiet.DaGiao = itm.SoLuong
            Next
        End If

        If dt.tbPhieuXuats.Where(Function(s) s.idKhachHangDatHang = KhachHangDatHang.id).Count > 0 Then
            For Each itm In dt.tbPhieuXuats.Where(Function(s) s.idKhachHangDatHang = KhachHangDatHang.id)
                For Each im In dt.tbChiTietPhieuXuats.Where(Function(s) s.idPhieuXuat = itm.id)
                    lstChiTietPhieuXuat.Add(im)
                Next
            Next
            Dim rls = From itm In lstChiTietPhieuXuat
                      Group By itm.idSanPham Into SoLuong = Sum(itm.SoLuong)
            For Each itm In rls
                Dim Chitiet = lstChiTietDatHang.Single(Function(s) s.idSanPham = itm.idSanPham)
                Chitiet.DaGiao = itm.SoLuong
            Next
        End If
        For Each itm In lstChiTietDatHang
            itm.Con = itm.SoLuong - itm.DaGiao
            itm.TonKho = itm.tbSanPham.SoLuong
            itm.KhoiLuongRieng = itm.tbSanPham.KhoiLuongRieng
        Next
...
#Region "ĐỒNG Ý ĐƠN ĐẶT HÀNG"
    Public Sub btnDongY_Click(sender As Object, e As EventArgs) Handles btnDongY.Click
        Dim DatHangMoi = SuaKhachHangDatHang()
    ...
    '    If isSuaPhieu Then
    '    Else
    '        Dim DatHangDaTao = DongYKhachHangDatHang()
    '        If DatHangDaTao IsNot Nothing Then
    '            'in trực tiếp phiếu ra
    '            mdlGlobals.InKhachHangDatHang(DatHangDaTao)
    '            If My.Settings.HienFormSauKhiIn Then
    '                Dim frm As New frmInKhachHangDatHang
    '                frm.DatHang = DatHangDaTao
    '                frm.ShowDialog()
    '            End If
    '        End If
    '    End If
    ...
    Private Function SuaKhachHangDatHang() As tbKhachHangDatHang
        ...
        If My.Settings.LuonXacNhanHoaDon Then
            If ComponentFactory.Krypton.Toolkit.KryptonMessageBox.Show("Xác nhận sửa đơn đặt hàng " + txtSoPhieuNhap.Text.Trim, "Xác nhận đặt hàng", MessageBoxButtons.YesNo, MessageBoxIcon.Question) <> DialogResult.Yes Then
                Return Nothing
            End If
        End If
        With DonDatHang
            ...
            For Each itm In lstChiTietDatHang
                If itm.id = 0 Then
                    .tbChiTietKhachHangDatHangs.Add(itm)
                End If
            Next
        End With
        Try
        ...
        Catch ex As Exception
            ComponentFactory.Krypton.Toolkit.KryptonMessageBox.Show("Sửa phiếu đặt hàng thất bại. Vui lòng thử lại" + ex.Message, "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information)
            undoChange()
            Return Nothing
        End Try
    End Function
...
#Region "CẬP NHẬT SỐ PHIẾU"
    Public Sub btnCapNhapSoPhieu_Click(sender As Object, e As EventArgs) Handles btnCapNhapSoPhieu.Click
        If isNhapHang = False Then
            txtSoPhieuNhap.Text = dtpNgayLap.Value.ToString("MMyy") + "D" + txtSoPhieuNhap.Tag.ToString.PadLeft(4, "0")
        Else
            txtSoPhieuNhap.Text = dtpNgayLap.Value.ToString("MMyy") + "P" + txtSoPhieuNhap.Tag.ToString.PadLeft(4, "0")
        End If
    End Sub
...
#Region "CAP NHAT / TIM KIEM KHÁCH HÀNG"
    ...
    Private Sub CapNhatKhachHang(key As String)
        ...
        ctrl.bsKhachHang.DataSource = rls
    End Sub
...
#End Region
    ...
    Private Sub dtpNgayNiemYet_KeyPress(sender As Object, e As KeyPressEventArgs) Handles dtpNgayDuKienGiaoHang.KeyPress
        ...
        End If
    End Sub
    Dim isSuaPhieu As Boolean = False
    ...
    Private Sub btnUndochange_Click(sender As Object, e As EventArgs) Handles btnUndochange.Click
        undoChange()
        XemPhieu(DonDatHang)
    End Sub

    Private Sub ctrlSuaKhachHangDatHang_ControlRemoved(sender As Object, e As ControlEventArgs) Handles MyBase.ControlRemoved
        undoChange()
    End Sub
    ...
    Private Sub ToolStripButton2_Click(sender As Object, e As EventArgs) Handles ToolStripButton2.Click
        SuaKhachHangDatHang()
        Dim vKhachHangDatHang As vwKhachHangDatHang = dt.vwKhachHangDatHangs.Single(Function(s) s.id = DonDatHang.id)
